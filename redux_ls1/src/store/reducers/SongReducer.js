import { ADD_SONGS, DELETE_SONG, FIND_SONG } from "../types";
...
const songReducer = (state = initialState, action ) => {
  switch (action.type) {
    case ADD_SONGS:
      return {
        ...state,
        listSongs: [...state.listSongs, action.payload],
      };

    case DELETE_SONG:
      return {
        ...state,
        listSongs: state.listSongs.filter(
          (song) => song.songId !== action.payload
        ),
      };
    case FIND_SONG:
      const {value} = action;
      const listSongFind = state.listSongs.filter((val) => val.songName.includes(value));
      return {...state, value, listSongFind};  
    default:
      return state;
  }
};
